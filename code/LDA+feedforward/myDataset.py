...
class myDataset(Dataset):
    ...
    def __init__(self, data_path, max_vocab=100000, max_len=512):
        ...
        #corpus=df.tfidf_vector
        lda = LdaModel(corpus, id2word=dictionary, num_topics=7,iterations = 800,chunksize = 150,passes = 40 )
        corpus_topic = lda[corpus]
        numpy_matrix = gensim.matutils.corpus2dense(corpus_topic, num_terms=7)
        numpy_matrix = numpy_matrix.T
        topic_list = np.array(lda.print_topics(10))

        print(topic_list)
        print(common_texts[44])
        test_doc = common_texts[44]
        test_lda=numpy_matrix[44][:]
        test_lda=np.array(test_lda).reshape(7)
        print(test_doc)
        print(test_lda)
